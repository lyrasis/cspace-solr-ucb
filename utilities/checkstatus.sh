...
do
    ...
    do
       if [[ -e ${HOME}/logs/${t}.solr_extract_${d}.log ]]
       then
           CORE="${t}-${d}"
           DATE=`tail -1 ${HOME}/logs/${t}.solr_extract_${d}.log | perl -ne 'if (/(Sun|Mon|Tue|Wed|Thu|Fri|Sat)/){print}else{print "PROBLEM: log file does not show a date as last line"}'`
           ...
           STATUS=`grep '"status":' ${HOME}/logs/${t}.solr_extract_${d}.log | tail -1 | perl -ne 'unless (/.status.:0/) {print "\nNon-zero status from Solr:\n$_"}'`

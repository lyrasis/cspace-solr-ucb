    ...
    for cells in inputfile:
        ...
        if name in parsednames:
            ...
            response = requests.get(tropicosURL,
                                    params={'name': name.replace('.', '%2E'), 'pagesize': numberWanted,
                                            'apikey': tropicos_api_key, 'format': 'json'})
            # loginfo('taxoneditor', tropicosURL, {}, {})
            response.encoding = 'utf-8'
            try:
                names2use = response.json()
                if 'Error' in names2use[0]:
                    if names2use[0]['Error'] != 'No names were found':
                        print('taxoneditor', 'ERROR: from Tropicos: %s' % names2use['Error'])
                    names2use = []
            except:
                print('taxoneditor', 'ERROR: could not parse returned Tropicos JSON, or it was empty')
                names2use = []
